...
只使用行最小值列递增性质的算法在最小元素都位于第一列的情形下，每一行都需要遍历所有n列，复杂度会达到$O(mn)$。而由于Monge矩阵的转置仍为Monge矩阵，Monge矩阵有列最小值行递增的性质，若算法同时考虑行最小值列递增和列最小值行递增，能够巧妙避开了这个问题，减少元素比较次数，提高算法效率。因此，在测试一般情况下以后，测试算例取最小元素都位于第一列的Monge矩阵，对比两种算法效率和比较次数。结果如下：
