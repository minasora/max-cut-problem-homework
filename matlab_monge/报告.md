...
可以看出，对于不同维数下的Monge矩阵，算法的平均计算时间与比较次数都明显小于通用最小元素算法。同时，随着维数的增加，相比于通用算法，该算法的计算次数并没有发生显著的增加。该试验结果可以证实我们之前的结论，即该算法的整体效率要优于通用算法。

#### 最小元素均位于第一列情况下的效率测试

由于只使用最小列递增性质的算法在最小元素都位于第一列的情形下复杂度会达到$O(mn)$，即全部遍历，而行列并用则利用Monge矩阵的转置也是Monge矩阵的性质巧妙避开了这个问题。因此，在测试一般情况下以后，我们也测试了最小元素都位于第一列的情况。结果如下：
...
根据测试结果，算法的平均计算时间与比较次数都小于通用算法。因此没有出现只使用最小列递增性质的算法效率相比通用算法没有明显改善的情况。可以说明行列并用的算法相比只使用最小列递增性质的算法，整体效率会更高一些。
