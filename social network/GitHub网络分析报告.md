...
编写代码进行计算，得到结果如下（按authority进行排序）：
...
从结果可以看出，由于改进的HITS考虑了用户自身的质量，将开发者获得的总star数纳入影响力的考虑范围，影响力前10名者不再与入度与出度高度相关，其结果相比直接使用中心度指标进行影响力分析有了改进。因此，可以认为，改进的HITS算法得到的authority值可以很好地作为评价开发者影响力的指标。
